describe('User管理APIのテスト', function(){
  ...
  it('ユーザーIDで取得できる', function(done){
    if (__karma__.config.login == 'anonymous') {
      var user = AppPot.getAnonymousUser();
      AppPot.User.findById(user.userId)
        .then(function(user){
          expect(user instanceof AppPot.User).toBeTruthy();
          expect(user.firstName).toEqual('Anonymouse User');
          expect(user.lastName).toEqual('Anonymouse User');
          expect(user.groupsRoles instanceof Array).toBeTruthy();
          done();
        });
    } else {
  ...
  it('ユーザー一覧を取得できる', function(done){
    if (__karma__.config.login == 'anonymous') {
      var groupId = AppPot.getAnonymousUser().groupsRoles[0].groupId;
      AppPot.User.list(groupId)
        .then(function(users){
          ...
          expect(users[0].groupsRoles[0].groupName).toEqual('Anonymous User Group');
          done();
        });
    } else {
