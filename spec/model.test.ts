import { AppPot, TaskModel, PlaceModel, account } from "./config";

describe("Modelのテスト", function () {
  beforeAll(async () => {
    await AppPot.LocalAuthenticator.login(account.username, account.password);
  });
  ...
  beforeEach(function (done) {
    ...
    this.numOfTask = 20;
    this.placeIds = [];
    AppPot.dropAndCreateDatabase([TaskModel, PlaceModel])
      .then(function () {
        var models = [];
  ...
  it("defineModelで定義したメソッドが使用できる", function (done) {
    ...
    TaskModel.findById(self.testTarget.objectId)
      .then(function (model) {
        var testArg = 123;
        expect(model.testMethod(testArg)).toBe(
          model.limit + "-" + model.num + "-" + testArg
        );
        done();
      })
      .catch(function (err) {
